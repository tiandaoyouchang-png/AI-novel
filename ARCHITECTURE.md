...
The `reviewed -> drafted` edge supports bounded repair. The next chapter stays locked until continuity is committed.
...
- bounded context compilation using contract IDs, terms, active continuity, and the prior chapter handoff;
...
- retrieval indexes for book-scale search beyond the current YAML selector;
- EPUB/DOCX and platform-specific publishing packages;
- opening-hook blind-test records before foundation approval;
- scene, subplot, and character-arc beat grids;
