...
function isPassFailLine(line) {
    const up = line.toUpperCase();
    return up === "PASS" || up === "FAIL";
}
export function createNovelTpPlan(ctx) {
    return tool({
        ...
        async execute(args, toolContext) {
            ...
            const councilSystem = "You are COUNCIL (discussion only). Propose a plan; do NOT write final bible files. Be concrete and list risks.";
            const writerSystem = "You are WRITER. Produce the final Bible markdown contents ONLY (no commentary). Follow constraints/style_profile.";
            const reviewerSystem = "You are REVIEWER (strict gate). Output first line PASS or FAIL. You MUST NOT write prose; only review.";
            const baseContext = [
                "BRIEF:",
                brief.trim(),
                "",
