...
function isPass(line: string) {
  return line.trim().toUpperCase() === "PASS";
}

function isPassFail(line: string) {
  const up = line.trim().toUpperCase();
  return up === "PASS" || up === "FAIL";
}

function extractChapterBeat(allBeats: string, chId: string) {
...
function takePrefix(s: string, maxChars: number) {
  const t = s.trim();
  if (t.length <= maxChars) return t;
  return t.slice(0, maxChars) + "\n... (truncated)";
}

export function createNovelTpChapter(ctx: ToolCtx) {
  return tool({
    description:
      "Stage 2+3: council->writer->reviewer loop -> auditors(3/3) -> character update -> humanizer -> recheck -> final.",
    args: {
    ...
    async execute(
      ...
      const writerSystem =
        "You are WRITER. Output ONLY the chapter prose in Markdown (no commentary). Respect constraints/style_profile and canon.";
      const reviewerSystem =
        "You are REVIEWER (strict gate). Output first line PASS or FAIL; then blocking issues + executable fix instructions. No rewriting.";
      const auditorSystem =
        "You are AUDITOR (strict final gate). Output first line PASS or FAIL; then blocking issues + executable fix instructions. No rewriting.";
      const humanizerSystem =
        "You are HUMANIZER. Only change expression; do NOT change facts/causality/canon. Output ONLY the revised chapter Markdown.";
      ...
      const deltaSystem =
        "You extract character deltas. Output markdown only. Include: status/relationships/goals/info_boundary/resources changes.";
      const deltaPrompt = [
        "CONTEXT:\n" + baseContext,
        "\nCHAPTER (prefinal):\n" + prefinalText.trim(),
      ...
      for (let round = 1; round <= Math.max(1, Math.min(10, args.maxHumanizeRounds)); round++) {
        ...
        const recheckSystem =
          "You are QUICK_RECHECK. Output first line PASS or FAIL. Check POV drift, banned words, profanity, new facts, info leaks, style mismatch.";
        const recheckPrompt = [
          "CONSTRAINTS:\n" + constraints.trim(),
          "\nSTYLE_PROFILE:\n" + style.trim(),
