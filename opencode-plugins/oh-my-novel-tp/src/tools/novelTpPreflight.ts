...
type MissingItem = {
  file: "constraints" | "style_profile" | "brief";
  key: string;
  message: string;
};
...
function lineValue(text: string, key: string) {
...
function hasAnyNonPlaceholder(text: string, key: string) {
  const v = lineValue(text, key);
  if (!v) return false;
  if (v === "(TODO)" || v === "TODO") return false;
  return true;
}

export function createNovelTpPreflight(ctx: ToolCtx) {
  return tool({
    ...
    async execute(args: { root?: string }) {
      ...
      if (styleText) {
        ...
        for (const k of required) {
          if (hasAnyNonPlaceholder(styleText, k)) continue;
        ...
        if (required.some((k) => !hasAnyNonPlaceholder(styleText, k))) {
          questions.push({
            ...
            question:
              "本次 style_profile 缺少必填特征参数（sentence_rhythm/diction_preferences/dialogue_style/narration_and_inner_monologue）。请逐项给一句话配置。",
            options: []
          });
        }
      }
