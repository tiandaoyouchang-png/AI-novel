...
Status note for `0.4.0`: points 1–10 now have tested command-line implementations. The new premise/scope/capability/evidence/reveal gates also prevent a strong occupational premise from drifting into generic escalation. Reader tests and publication metrics remain author-supplied and local; the system never labels an AI judgment as market proof or silently rewrites canon from metrics.
...
| 7 | Context selection lacked dynamic character/plot state and explicit secret visibility | Sudowrite saliency supports card/trait visibility to prevent premature disclosure | Dynamic character/story cards, death-state blocking, knowledge separation, graded evidence, and chapter-number reveal policy | Implemented | Dead onstage participants are blocked; protected evidence is redacted and excluded from semantic retrieval; reveal prerequisites and windows are validated |
