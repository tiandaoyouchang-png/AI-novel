...
> 当前版本：`0.4.0`，处于可运行的工程验证阶段。项目尚未接入番茄小说或知乎盐选的自动发布接口，也不承诺平台流量、签约结果或作品收入。
...
| `evidence.yaml` | 异常线索、方向证据、关联证据、定案证据、伏笔证据及其验证局限 |
...
`planning/story-guardrails.yaml` 在基础设定批准前锁定：

- 一句话核心设定与本书招牌破局机制；
- 本卷允许的最大案件规模和幕后层级；
- 每个主要人物允许、禁止、必须依赖他人的能力；
- 每章巧合预算；
- 时代内制度、物理规则和禁用现代词；
- 配角的独立目标与不可替代领域；
- 失败不能自动变成奖励，以及必须保留的永久代价；
- 禁止使用的剧情捷径。

新章节使用 schema-v3 合同，必须把异常、至少两种解释、排除测试、结论和局限写清楚；同时声明证据动作、章节规模、人物能力、配角独立决定、揭秘 ID、巧合、结局代价和对手的合理反制。越过县域规模、临时增加剑术、提前揭秘或让调查物直接写出凶手名字，都会在进入“已规划”状态前被阻断。

`planning/reveal-policy.yaml` 为重要真相设置最早、目标和最晚揭示章节，并绑定前置证据。超过最晚章节仍未兑现会阻断检查；需要延期时必须显式标记并记录原因。`continuity/evidence.yaml` 对每条证据标注层级、状态、支持/反驳的主张、验证方法和证明局限。到达揭示章节以前，上下文会隐藏受保护证据的真实含义；章节提交时，揭秘状态与连续性文件在同一个可恢复事务中更新。
...
已提交的旧章节仍可保留 schema-v2 合同和审稿记录；从升级后的下一章开始必须使用 schema-v3。旧工作区需要由作者或 Codex 明确补齐，系统不会根据既有正文擅自猜测边界：

1. 参照 [`examples/commercial-demo/planning/story-guardrails.yaml`](examples/commercial-demo/planning/story-guardrails.yaml) 创建核心设定、规模、能力、时代和代价门禁；
2. 创建 `planning/reveal-policy.yaml`，长篇至少安排一个重要揭秘；
3. 创建 `continuity/evidence.yaml`；可先为空，也可从已验收正文提取分级证据；
4. 为后续章节改用 schema-v3 合同和十项 schema-v3 审稿；
5. 显式作废并重新批准基础设定和当前卷计划。

```bash
node plugins/codex-novel/dist/novelctl.cjs invalidate novels/my-story \
  --artifact foundation \
  --reason "升级 v0.4 核心设定、证据与揭秘门禁"
node plugins/codex-novel/dist/novelctl.cjs phase novels/my-story --to foundation_approved
node plugins/codex-novel/dist/novelctl.cjs phase novels/my-story --to production
```

重新批准只更新规划指纹，不会改写已验收正文；正式操作前仍建议先创建命名修订版本。
...
```text
schema-v3 章节合同：场景细纲、证据链、能力、规模、揭秘、代价
  → 有界上下文
  → 草稿
  → 机械质量检查
  → 结构化审稿
...
章节审查必须分别给出人物声音、信息边界、场景价值变化、核心设定、规模纪律、能力边界、证据链、时代可信度、配角自主性和永久代价的正文证据。任何修改都必须重新运行完整质量检查和审稿，不能只验证被修的局部。通过审查后，`handoff.yaml` 会记录已解决事项、未解决事项、角色承接状态、情绪承接和下一章约束，并绑定最终正文指纹。
